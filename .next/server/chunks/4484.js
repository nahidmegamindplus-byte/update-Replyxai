"use strict";exports.id=4484,exports.ids=[4484],exports.modules={84517:(e,a,t)=>{t.d(a,{S:()=>l});var T=t(9487),E=t(11671),s=t.n(E),i=t(21431);let n=globalThis;async function o(){if(!n.__isHealing){n.__isHealing=!0,console.log("[DB Health] Running automatic SQLite self-healing & maintenance...");try{try{await T.default.$queryRawUnsafe("PRAGMA wal_checkpoint(TRUNCATE);")}catch(e){}try{await T.default.$queryRawUnsafe("REINDEX;")}catch(e){}try{await T.default.$queryRawUnsafe("VACUUM;")}catch(e){}console.log("[DB Health] Database self-healing completed successfully.")}catch(e){console.error("[DB Health] Healing warning:",e)}finally{n.__isHealing=!1}}}async function l(){if(!n.__isDbInitialized)return n.__initPromise||(n.__initPromise=(async()=>{try{try{await T.default.$queryRawUnsafe("PRAGMA journal_mode = WAL;")}catch(e){}try{await T.default.$queryRawUnsafe("PRAGMA busy_timeout = 30000;")}catch(e){}try{await T.default.$queryRawUnsafe("PRAGMA synchronous = NORMAL;")}catch(e){}try{await T.default.$queryRawUnsafe("PRAGMA foreign_keys = ON;")}catch(e){}await T.default.$executeRawUnsafe(`
...
Please craft the Step #${t.stepNumber} follow-up message now:`;if("GEMINI"===a){let a=e.geminiKey||process.env.GEMINI_API_KEY||"";if(a){let e=new r.$D(a).getGenerativeModel({model:i||"gemini-1.5-flash",systemInstruction:U,generationConfig:{temperature:.75,maxOutputTokens:250}}),t=(await e.generateContent(A)).response.text().trim().replace(/^["']|["']$/g,"");if(t)return{text:t,model:`GEMINI:${i||"gemini-1.5-flash"}`}}}else if("OPENAI"===a){let a=e.openaiKey||process.env.OPENAI_API_KEY||"";if(a){let e=new L.ZP({apiKey:a}),t=await e.chat.completions.create({model:i||"gpt-4o-mini",messages:[{role:"system",content:U},{role:"user",content:A}],temperature:.75,max_tokens:250}),T=t.choices[0]?.message?.content?.trim().replace(/^["']|["']$/g,"");if(T)return{text:T,model:`OPENAI:${i||"gpt-4o-mini"}`}}}else if("DEEPSEEK"===a||"GOROUTER"===a||"OPENROUTER"===a){let t="DEEPSEEK"===a?e.deepseekKey||process.env.DEEPSEEK_API_KEY||"":e.gorouterKey||process.env.GOROUTER_API_KEY||"",T="DEEPSEEK"===a?"https://api.deepseek.com/v1":e.gorouterBaseUrl;if(t){let e=new L.ZP({apiKey:t,baseURL:T}),E=await e.chat.completions.create({model:i||("DEEPSEEK"===a?"deepseek-chat":"deepseek/deepseek-chat"),messages:[{role:"system",content:U},{role:"user",content:A}],temperature:.75,max_tokens:250}),s=E.choices[0]?.message?.content?.trim().replace(/^["']|["']$/g,"");if(s)return{text:s,model:`${a}:${i||"chat"}`}}}}catch(e){i.LI.warn("AI follow-up generation failed, falling back to smart dynamic template:",e)}let d="";switch(t.stepNumber){case 1:d=`আসসালামু আলাইকুম ${N}! আপনার পছন্দের পণ্যটি নিয়ে কোনো প্রশ্ন ছিল কি? কোনো সহায়তা লাগলে জানাবেন, আমরা এখনই অর্ডার কনফার্ম করে দিচ্ছি! 😊🛍️`;break;case 2:d=`প্রিয় ${N}, আশা করি ভালো আছেন। আমাদের পণ্যটিতে ক্যাশ অন ডেলিভারি (COD) এবং দ্রুত ডেলিভারির সুবিধা রয়েছে। আপনার অর্ডারটি কি কনফার্ম করে দেব? 📦✨`;break;case 3:d=`হ্যালো ${N}! আপনার পছন্দের প্রোডাক্টটির স্টক কিন্তু সীমিত। আপনি চাইলে আপনার জন্য স্টক হোল্ড করে রাখতে পারি। জানাতে পারেন! 😊`;break;case 4:d=`আসসালামু আলাইকুম ${N}, কোনো বিশেষ অফার বা অন্য কোনো প্রোডাক্ট দেখতে চাইলে আমাদের জানাতে পারেন। আপনার সেবায় আমরা সর্বদা প্রস্তুত! 🌟`;break;default:d=`শ্রদ্ধেয় ${N}, আমাদের সাথে যুক্ত থাকার জন্য আন্তরিক ধন্যবাদ। যেকোনো সময় পণ্য বা সেবার জন্য আমাদের মেসেজ দিতে পারেন। শুভকামনা! 💐`}return{text:d,model:"TEMPLATE_FALLBACK"}}function A(e){if(e.timeOfDay?.startsWith("MIN:")){let a=parseInt(e.timeOfDay.replace("MIN:",""),10);return isNaN(a)||a<=0?1:a}if(e.timeOfDay?.startsWith("HR:")){let a=parseInt(e.timeOfDay.replace("HR:",""),10);return isNaN(a)||a<=0?60:60*a}if(0===e.dayOffset&&e.timeOfDay?.includes(":")){let[a,t]=e.timeOfDay.split(":").map(Number);if(!isNaN(a)&&!isNaN(t))return Math.max(1,60*a+t)}return 1440*Math.max(1,e.dayOffset||1)}async function c(e){let a=[],t=0;try{await (0,o.S)();let n={followUpEnabled:!0,connectionStatus:{not:"DISCONNECTED"}};e&&(n.id=e);let r=await T.default.page.findMany({where:n});for(let e of r){let n=(0,E.pe)(e.pageAccessTokenEncrypted);if(!n)continue;let o={};if(e.extraConfig)try{o=JSON.parse(e.extraConfig)}catch(e){}if(!(await (0,l.Q)(e.userId)).eligible)continue;let r=await u(e.userId,e.id);if(0===r.length)continue;let L=new Date(Date.now()-3024e6);for(let E of(await T.default.conversation.findMany({where:{pageId:e.id,status:"ACTIVE",aiEnabled:!0,followUpStatus:{notIn:["COMPLETED","PAUSED","ORDER_PLACED","CUSTOMER_REPLIED"]},lastMessageAt:{gte:L}},include:{messages:{orderBy:{createdAt:"desc"},take:10},orders:{where:{createdAt:{gte:L}},take:1},followUpLogs:{orderBy:{createdAt:"asc"},take:10}},take:100}))){let L=E.channel||e.channel||"FACEBOOK",N=E.customerName||E.senderPsid;if(E.orders&&E.orders.length>0){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"ORDER_PLACED"}});continue}let d=E.messages[0];if(d&&"INCOMING"===d.direction){(E.currentFollowUpStep>0||E.followUpSentCount>0)&&await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"CUSTOMER_REPLIED"}});continue}let u=e.followUpMaxCount??5;if(u<999&&E.followUpSentCount>=u||"ONCE"===e.followUpFrequency&&E.followUpSentCount>=1){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"COMPLETED"}});continue}let c=E.currentFollowUpStep||0;if(c>=r.length){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"COMPLETED"}});continue}let f=r[c];if(e.followUpOnlySeen){if(!E.lastSeenAt)continue;if(0===c){let e=[...E.messages].reverse().find(e=>"OUTGOING"===e.direction);if(e&&new Date(E.lastSeenAt).getTime()<new Date(e.createdAt).getTime()-6e4)continue}}let O=Date.now(),I=!1;if(0===c){let a=e.followUpOnlySeen&&E.lastSeenAt?new Date(E.lastSeenAt).getTime():E.lastMessageAt?new Date(E.lastMessageAt).getTime():new Date(E.createdAt).getTime(),t=A(f);1!==f.dayOffset||f.timeOfDay?.startsWith("MIN:")||f.timeOfDay?.startsWith("HR:")||!((e.followUpWaitMinutes??30)<1440)||(t=e.followUpWaitMinutes??30);let T=6e4*Math.max(1,t);O-a>=T&&(I=!0)}else if(E.lastFollowUpSentAt){let a=new Date(E.lastFollowUpSentAt).getTime(),t=r[c-1],T=A(f),s=t?A(t):0,i=T>s?T-s:T;"DAILY"!==e.followUpFrequency||f.timeOfDay?.startsWith("MIN:")?"CUSTOM_INTERVAL"===e.followUpFrequency&&!f.timeOfDay?.startsWith("MIN:")&&e.followUpIntervalHours&&f.dayOffset>0&&(i=Math.max(i,60*e.followUpIntervalHours)):i=Math.max(i,1440);let n=6e4*Math.max(1,i);O-a>=n&&(I=!0)}else I=!0;if(!I)continue;let p="",w="CUSTOM_TEMPLATE";if(e.followUpMessage&&e.followUpMessage.trim().length>0){let a=E.customerName?.split(" ")[0]||"গ্রাহক";p=e.followUpMessage.replace(/{name}/gi,a).replace(/{customer_name}/gi,a).replace(/{page_name}/gi,e.pageName)}else{let a=(E.followUpLogs||[]).map(e=>e.messageText).filter(e=>"string"==typeof e&&e.trim().length>0),t=[...E.messages].reverse().map(e=>({direction:e.direction,text:e.messageText||""})),T=await U({conversationId:E.id,customerName:E.customerName,step:f,conversationHistory:t,previousFollowUps:a,pageName:e.pageName,replyLanguage:e.replyLanguage||"বাংলা",businessInstructions:e.aiInstructions||""});p=T.text,w=T.model}if(!p||0===p.trim().length)continue;let R=await (0,s.Wi)({channel:L,recipientId:E.senderPsid,text:p,accessToken:n,channelIdentifier:e.channelIdentifier||e.facebookPageId,extraConfig:o});if(R.success){t++;let s=c+1>=r.length||"ONCE"===e.followUpFrequency,n=s?"COMPLETED":"IN_PROGRESS",o=c+1,d=null;if(!s){let a="CUSTOM_INTERVAL"===e.followUpFrequency&&e.followUpIntervalHours||24;d=new Date(Date.now()+36e5*a)}await T.default.followUpLog.create({data:{userId:e.userId,pageId:e.id,conversationId:E.id,stepNumber:f.stepNumber,dayOffset:f.dayOffset,scheduledTime:f.timeOfDay,messageText:p,channel:L,customerName:E.customerName,senderPsid:E.senderPsid,status:"SENT",aiModel:w}}),await T.default.message.create({data:{conversationId:E.id,userId:e.userId,pageId:e.id,senderPsid:E.senderPsid,direction:"OUTGOING",messageType:"TEXT",messageText:p,aiGenerated:!0,aiModel:w}}),await T.default.conversation.update({where:{id:E.id},data:{currentFollowUpStep:o,followUpStatus:n,lastFollowUpSentAt:new Date,lastMessage:p,lastMessageAt:new Date,followUpSentCount:{increment:1},nextFollowUpDueAt:d}}),await (0,l.g)(e.userId),await (0,i.ag)({userId:e.userId,pageId:e.id,action:"FOLLOW_UP_SENT",description:`স্বয়ংক্রিয় ধাপ #${f.stepNumber} ফলো-আপ পাঠানো হয়েছে (${N}, ${L})`}),a.push({pageId:e.id,conversationId:E.id,customer:N,status:"SENT",channel:L,stepNumber:f.stepNumber}),i.LI.info(`[Follow-Up] Step #${f.stepNumber} sent to ${N} (${L})`)}else await T.default.followUpLog.create({data:{userId:e.userId,pageId:e.id,conversationId:E.id,stepNumber:f.stepNumber,dayOffset:f.dayOffset,scheduledTime:f.timeOfDay,messageText:p,channel:L,customerName:E.customerName,senderPsid:E.senderPsid,status:`FAILED: ${R.error||"API Error"}`,aiModel:w}}),a.push({pageId:e.id,conversationId:E.id,customer:N,status:`FAILED: ${R.error||"API Error"}`,channel:L,stepNumber:f.stepNumber})}}return{scannedPages:r.length,sentCount:t,results:a}}catch(e){return i.LI.error("Error running follow-up automation:",e),{scannedPages:0,sentCount:0,results:a}}}function f(e=6e4){!N.followUpWorkerInterval&&(i.LI.info(`[Follow-Up Worker] Background automation engine active (interval: ${e/1e3}s)`),N.followUpWorkerInterval=setInterval(async()=>{if(!N.isFollowUpWorkerRunning){N.isFollowUpWorkerRunning=!0;try{let e=await c();e.sentCount>0&&i.LI.info(`[Follow-Up Worker] Dispatched ${e.sentCount} follow-up message(s)`)}catch(e){i.LI.error("[Follow-Up Worker] Scheduled run error:",e)}finally{N.isFollowUpWorkerRunning=!1}}},e),N.followUpWorkerInterval&&"function"==typeof N.followUpWorkerInterval.unref&&N.followUpWorkerInterval.unref())}}};