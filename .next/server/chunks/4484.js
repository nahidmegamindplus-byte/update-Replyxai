...
${N||"Cash on Delivery (COD) available all over Bangladesh. Fast delivery."}
`,A=`[CUSTOMER NAME]: ${r}
[PREVIOUS CHAT HISTORY]:
${U||"Customer previously greeted and asked for product information."}

Please craft the Step #${a.stepNumber} follow-up message now:`;if("GEMINI"===t){let t=e.geminiKey||process.env.GEMINI_API_KEY||"";if(t){let e=new i.$D(t).getGenerativeModel({model:n||"gemini-1.5-flash",systemInstruction:u,generationConfig:{temperature:.75,maxOutputTokens:250}}),a=(await e.generateContent(A)).response.text().trim().replace(/^["']|["']$/g,"");if(a)return{text:a,model:`GEMINI:${n||"gemini-1.5-flash"}`}}}else if("OPENAI"===t){let t=e.openaiKey||process.env.OPENAI_API_KEY||"";if(t){let e=new l.ZP({apiKey:t}),a=await e.chat.completions.create({model:n||"gpt-4o-mini",messages:[{role:"system",content:u},{role:"user",content:A}],temperature:.75,max_tokens:250}),T=a.choices[0]?.message?.content?.trim().replace(/^["']|["']$/g,"");if(T)return{text:T,model:`OPENAI:${n||"gpt-4o-mini"}`}}}else if("DEEPSEEK"===t||"GOROUTER"===t||"OPENROUTER"===t){let a="DEEPSEEK"===t?e.deepseekKey||process.env.DEEPSEEK_API_KEY||"":e.gorouterKey||process.env.GOROUTER_API_KEY||"",T="DEEPSEEK"===t?"https://api.deepseek.com/v1":e.gorouterBaseUrl;if(a){let e=new l.ZP({apiKey:a,baseURL:T}),E=await e.chat.completions.create({model:n||("DEEPSEEK"===t?"deepseek-chat":"deepseek/deepseek-chat"),messages:[{role:"system",content:u},{role:"user",content:A}],temperature:.75,max_tokens:250}),s=E.choices[0]?.message?.content?.trim().replace(/^["']|["']$/g,"");if(s)return{text:s,model:`${t}:${n||"chat"}`}}}}catch(e){n.LI.warn("AI follow-up generation failed, falling back to smart dynamic template:",e)}let U="";switch(a.stepNumber){case 1:U=`আসসালামু আলাইকুম ${r}! আপনার পছন্দের পণ্যটি নিয়ে কোনো প্রশ্ন ছিল কি? কোনো সহায়তা লাগলে জানাবেন, আমরা এখনই অর্ডার কনফার্ম করে দিচ্ছি! 😊🛍️`;break;case 2:U=`প্রিয় ${r}, আশা করি ভালো আছেন। আমাদের পণ্যটিতে ক্যাশ অন ডেলিভারি (COD) এবং দ্রুত ডেলিভারির সুবিধা রয়েছে। আপনার অর্ডারটি কি কনফার্ম করে দেব? 📦✨`;break;case 3:U=`হ্যালো ${r}! আপনার পছন্দের প্রোডাক্টটির স্টক কিন্তু সীমিত। আপনি চাইলে আপনার জন্য স্টক হোল্ড করে রাখতে পারি। জানাতে পারেন! 😊`;break;case 4:U=`আসসালামু আলাইকুম ${r}, কোনো বিশেষ অফার বা অন্য কোনো প্রোডাক্ট দেখতে চাইলে আমাদের জানাতে পারেন। আপনার সেবায় আমরা সর্বদা প্রস্তুত! 🌟`;break;default:U=`শ্রদ্ধেয় ${r}, আমাদের সাথে যুক্ত থাকার জন্য আন্তরিক ধন্যবাদ। যেকোনো সময় পণ্য বা সেবার জন্য আমাদের মেসেজ দিতে পারেন। শুভকামনা! 💐`}return{text:U,model:"TEMPLATE_FALLBACK"}}async function A(e){let t=[],a=0;try{await (0,o.S)();let L={followUpEnabled:!0,connectionStatus:{not:"DISCONNECTED"}};e&&(L.id=e);let i=await T.default.page.findMany({where:L});for(let e of i){let L=(0,E.pe)(e.pageAccessTokenEncrypted);if(!L)continue;let o={};if(e.extraConfig)try{o=JSON.parse(e.extraConfig)}catch(e){}let i=await d(e.userId,e.id);if(0===i.length)continue;let l=new Date(Date.now()-3024e6);for(let E of(await T.default.conversation.findMany({where:{pageId:e.id,status:"ACTIVE",aiEnabled:!0,followUpStatus:{notIn:["COMPLETED","PAUSED","ORDER_PLACED","CUSTOMER_REPLIED"]},lastMessageAt:{gte:l}},include:{messages:{orderBy:{createdAt:"desc"},take:10},orders:{where:{createdAt:{gte:l}},take:1},followUpLogs:{orderBy:{createdAt:"asc"},take:10}},take:100}))){let l=E.channel||e.channel||"FACEBOOK",N=E.customerName||E.senderPsid;if(E.orders&&E.orders.length>0){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"ORDER_PLACED"}});continue}let U=E.messages[0];if(U&&"INCOMING"===U.direction){(E.currentFollowUpStep>0||E.followUpSentCount>0)&&await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"CUSTOMER_REPLIED"}});continue}let d=e.followUpMaxCount??5;if(d<999&&E.followUpSentCount>=d||"ONCE"===e.followUpFrequency&&E.followUpSentCount>=1){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"COMPLETED"}});continue}let A=E.currentFollowUpStep||0;if(A>=i.length){await T.default.conversation.update({where:{id:E.id},data:{followUpStatus:"COMPLETED"}});continue}let c=i[A];if(e.followUpOnlySeen){if(!E.lastSeenAt)continue;let e=E.messages.find(e=>"OUTGOING"===e.direction);if(e&&new Date(E.lastSeenAt).getTime()<new Date(e.createdAt).getTime()-1e4)continue}let O=Date.now(),f=!1;if(0===A){let t=e.followUpOnlySeen&&E.lastSeenAt?new Date(E.lastSeenAt).getTime():E.lastMessageAt?new Date(E.lastMessageAt).getTime():new Date(E.createdAt).getTime(),a=r(c,e.followUpWaitMinutes??30),T=6e4*a;O-t>=T&&(f=!0)}else if(E.lastFollowUpSentAt){let t=new Date(E.lastFollowUpSentAt).getTime(),a=1440;if("DAILY"===e.followUpFrequency)a=1440;else if("CUSTOM_INTERVAL"===e.followUpFrequency)a=60*(e.followUpIntervalHours||24);else{let e=i[A-1],t=e?r(e,30):0,T=r(c,60);a=Math.max(1,T-t)}let T=6e4*Math.max(1,a);O-t>=T&&(f=!0)}else f=!0;if(!f)continue;let p="",I="CUSTOM_TEMPLATE";if(e.followUpMessage&&e.followUpMessage.trim().length>0){let t=E.customerName?.split(" ")[0]||"গ্রাহক";p=e.followUpMessage.replace(/{name}/gi,t).replace(/{customer_name}/gi,t).replace(/{page_name}/gi,e.pageName)}else{let t=(E.followUpLogs||[]).map(e=>e.messageText).filter(e=>"string"==typeof e&&e.trim().length>0),a=[...E.messages].reverse().map(e=>({direction:e.direction,text:e.messageText||""})),T=await u({conversationId:E.id,customerName:E.customerName,step:c,conversationHistory:a,previousFollowUps:t,pageName:e.pageName,replyLanguage:e.replyLanguage||"বাংলা",businessInstructions:e.aiInstructions||""});p=T.text,I=T.model}if(!p||0===p.trim().length)continue;let D=await (0,s.Wi)({channel:l,recipientId:E.senderPsid,text:p,accessToken:L,channelIdentifier:e.channelIdentifier||e.facebookPageId,extraConfig:o});if(D.success){a++;let s=A+1>=i.length||"ONCE"===e.followUpFrequency,L=s?"COMPLETED":"IN_PROGRESS",o=A+1,r=null;if(!s){let t="CUSTOM_INTERVAL"===e.followUpFrequency&&e.followUpIntervalHours||24;r=new Date(Date.now()+36e5*t)}await T.default.followUpLog.create({data:{userId:e.userId,pageId:e.id,conversationId:E.id,stepNumber:c.stepNumber,dayOffset:c.dayOffset,scheduledTime:c.timeOfDay,messageText:p,channel:l,customerName:E.customerName,senderPsid:E.senderPsid,status:"SENT",aiModel:I}}),await T.default.message.create({data:{conversationId:E.id,userId:e.userId,pageId:e.id,senderPsid:E.senderPsid,direction:"OUTGOING",messageType:"TEXT",messageText:p,aiGenerated:!0,aiModel:I}}),await T.default.conversation.update({where:{id:E.id},data:{currentFollowUpStep:o,followUpStatus:L,lastFollowUpSentAt:new Date,lastMessage:p,lastMessageAt:new Date,followUpSentCount:{increment:1},nextFollowUpDueAt:r}}),await (0,n.ag)({userId:e.userId,pageId:e.id,action:"FOLLOW_UP_SENT",description:`স্বয়ংক্রিয় ধাপ #${c.stepNumber} ফলো-আপ পাঠানো হয়েছে (${N}, ${l})`}),t.push({pageId:e.id,conversationId:E.id,customer:N,status:"SENT",channel:l,stepNumber:c.stepNumber}),n.LI.info(`[Follow-Up] Step #${c.stepNumber} sent to ${N} (${l})`)}else await T.default.followUpLog.create({data:{userId:e.userId,pageId:e.id,conversationId:E.id,stepNumber:c.stepNumber,dayOffset:c.dayOffset,scheduledTime:c.timeOfDay,messageText:p,channel:l,customerName:E.customerName,senderPsid:E.senderPsid,status:`FAILED: ${D.error||"API Error"}`,aiModel:I}}),t.push({pageId:e.id,conversationId:E.id,customer:N,status:`FAILED: ${D.error||"API Error"}`,channel:l,stepNumber:c.stepNumber})}}return{scannedPages:i.length,sentCount:a,results:t}}catch(e){return n.LI.error("Error running follow-up automation:",e),{scannedPages:0,sentCount:0,results:t}}}function c(e=6e4){!N.followUpWorkerInterval&&(n.LI.info(`[Follow-Up Worker] Background automation engine active (interval: ${e/1e3}s)`),N.followUpWorkerInterval=setInterval(async()=>{if(!N.isFollowUpWorkerRunning){N.isFollowUpWorkerRunning=!0;try{let e=await A();e.sentCount>0&&n.LI.info(`[Follow-Up Worker] Dispatched ${e.sentCount} follow-up message(s)`)}catch(e){n.LI.error("[Follow-Up Worker] Scheduled run error:",e)}finally{N.isFollowUpWorkerRunning=!1}}},e),N.followUpWorkerInterval&&"function"==typeof N.followUpWorkerInterval.unref&&N.followUpWorkerInterval.unref())}}};